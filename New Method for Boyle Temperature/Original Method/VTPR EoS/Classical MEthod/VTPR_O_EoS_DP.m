...
% Tc=305.3;Pc=49;w=0.099;Zc=0.279; % Ethane1
...
Delta_Helmholtz_Energy=zeros(n,1);Delta_Gibs=zeros(n,1);
Delta_Internal_Energy=zeros(n,1);RV=zeros(n,1);
Delta_Entropy=zeros(n,1);B=zeros(n,1);
Delta_Enthalpy=zeros(n,1);A=zeros(n,1);
Z=zeros(n,1);Tr=zeros(n,1);alpha=zeros(n,1);
F_B=zeros(n,1);C=zeros(n,1);DEI=zeros(n,1);
...
for i=1:n
    ...
    Delta_Internal_Energy(i)=(((a*(alpha(i)+(k*sqrt(Tr(i))*...
        sqrt(alpha(i)))))/(2*sqrt(2)*b))*...
    (log((OF(i)+c+((1-sqrt(2))*b))/(OF(i)+c+((1+sqrt(2))*b)))));
