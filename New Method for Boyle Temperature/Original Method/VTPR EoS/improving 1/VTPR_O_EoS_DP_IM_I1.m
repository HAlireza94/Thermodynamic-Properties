...
n1=numel(Tc);
P=1e-7;R=83.14472;
k=zeros(n1,1);c=zeros(n1,1);
b=zeros(n1,1);a=zeros(n1,1);
for i=1:n1
...
for i=1:n
     ...
     f=@(x) ((R*T(i))/(x+c(j)-b(j)))-((a(j)*((1+(k(j)*(1-((T(i)/Tc(j))^0.5))))^2))/...
         ((x+c(j))*(x+c(j)+b(j))-(b(j)*(x+c(j)-b(j)))))-P;
...
for i=1:n
  ...
  Delta_Enthalpy(i,j)=(R*T(i)*(Z(i,j)-1))+...
        (((a(j)*(alpha(i,j)+(k(j)*sqrt(Tr(i,j))*...
        sqrt(alpha(i,j)))))/(2*sqrt(2)*b(j)))*...
    (log((OF(i,j)+c(j)+((1-sqrt(2))*b(j)))/...
    (OF(i,j)+c(j)+((1+sqrt(2))*b(j))))));

    Delta_Entropy(i,j)=(R*(log(Z(i,j)+C(i,j)-B(i,j))))-...
        (((((a(j)*k(j)*sqrt(Tr(i,j))*...
        sqrt(alpha(i,j)))/T(i)))/(2*sqrt(2)*b(j)))*...
        (log((OF(i,j)+c(j)+((1+sqrt(2))*b(j)))/(OF(i,j)+c(j)+((1-sqrt(2))*b(j)))))); % Original
    
%     Delta_Internal_Energy(i,j)=(((a(j)*(alpha(i,j)+(k(j)*sqrt(Tr(i,j))*...
%         sqrt(alpha(i,j)))))/(2*sqrt(2)*b(j)))*...
%     (log((OF(i,j)+c(j)+((1-sqrt(2))*b(j)))/(OF(i,j)+c(j)+((1+sqrt(2))*b(j))))));

%       Delta_Helmholtz_Energy(i,j)=(R*T(i)*(log(1/(Z(i,j)+C(i,j)-B(i,j)))))-...
%         (((((a(j)*alpha(i,j)))/(2*sqrt(2)*b(j))))*...
%         (log((OF(i,j)+c(j)+((1-sqrt(2))*b(j)))/(OF(i)+c(j)+((1+sqrt(2))*b(j))))));
    
      SV(i,j)=b(j)-c(j)-((a(j)*alpha(i,j))/(R*T(i)));
    
      first_D_alpha(i,j)=(-k(j)/T(i))*sqrt(Tr(i,j))*sqrt(alpha(i,j));
    
      second_D_alpha(i,j)=((k(j)*((k(j)*sqrt(Tr(i,j)))+sqrt(alpha(i,j))))/(2*sqrt(Tc(j))*(T(i)^1.5)));
    
      theird_D_alpha(i,j)=(-0.75*k(j)*sqrt(Tr(i,j))*((k(j)*sqrt(Tr(i,j)))+sqrt(alpha(i,j))))/(T(i)^3);
    ...
    X20(i,j)=12.625250588376753*X18(i,j);
...
% for i=1:n1
%     
%     E(i,m)=(abs(T_Final(i,m)-T_B_Meng_Duan(i))/T_B_Meng_Duan(i))*100;
%         
% end
% 
% % Error(m)=(sum(E(:,m)))/60;
