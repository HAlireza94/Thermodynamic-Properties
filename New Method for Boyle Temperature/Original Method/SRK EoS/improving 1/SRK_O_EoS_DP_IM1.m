...
for i=1:n
     ...
     f=@(x) (((R*T(i))/(x-b(j)))-(((a(j)*((1+(k(j)*(1-sqrt(T(i)/Tc(j)))))^2))/(x*(x+b(j)))))-P); %$$$$$$$
...
Error=inf;
betta=linspace(-3,3,1000)';
gamma=linspace(-3,3,1000)';
n2=numel(betta);
...
%         +(((a(j)*alpha(i,j))/b(j))*(log((b(j)/(OF(i,j)))+1)));
    
    
       SV(i,j)=((0.08664*R*Tc(j))/Pc(j))-(((((0.42747*(R^2)*(Tc(j)^2)))/Pc(j))*...
        ((1+((0.480+(1.574*w(j))-(0.175*w(j)^2))*(1-(T(i)/Tc(j))^0.5)))^2))/(R*T(i)));
...
%    
%     
%     Delta_Gipss(i,j)=Delta_Enthalpy(i,j)-(T(i)*Delta_Entropy(i,j));
    ...
    for m=1:n2
     ...
     X20(i,j,m)=betta(m)*(X18(i,j)^betta(m));
