...
for i=1:j1
    for j=1:l1
        ...
        f=@(x)((R*T(j))/(x-b(i)))-((a(i)*alpha(j,i))/(((x+c(i))*(x+b(i)+(2*c(i))))))-P;
...
B(j,i)=(b(i)*P)/(R*T(j));
    ...
    H_R(j,i)=(R*T(j)*(Z(j,i)-1))-((A1(j,i)/((b(i)+c(i))))*log((Z(j,i)+C(j,i))/...
        (Z(j,i)+(2*C(j,i))+B(j,i))));
    
    A2(j,i)=a(i)*first_order_alpha(j,i);
    
    S_R(j,i)=(R*(log(Z(j,i)-B(j,i))))+((A2(j,i)/((b(i)+c(i))))*log(((Z(j,i)+(2*C(j,i))+B(j,i)))/(Z(j,i)+C(j,i))));
