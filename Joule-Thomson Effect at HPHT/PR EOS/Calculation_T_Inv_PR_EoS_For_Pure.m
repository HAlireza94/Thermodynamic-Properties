function T_inv=Calculation_T_Inv_PR_EoS_For_Pure(alpha_Function,w,Tc,Pc,T_B)
  ...
  switch lower(alpha_Function)
    ...
    case 'coquelet'
        ...
        % for calculation of the inversion temperature we must assume
        % temperaure ranges between Tc up to T_B+500
...
T_inv=fzero(F,T_B);
    ...
    case 'haghtalab'
     ...
     n2=1.7309+(1.6571*w)+(0.1554*w^2);
        ...
        ((R*x)^2))-((b-((a*(exp(1-(n2^(log(x/Tc))))))/(R*x)))/x);
        ...
        k1=0.003091+(0.013145*w);
         ...
         F=@(x)  -(((((a*(exp((k1*(x/Tc))+(k3*(1-((x/Tc)^0.5)))))*...
                ...
                (2*k1*sqrt(Tc)*(x^k2)))/(2*(Tc^(k2+1.5)))))*R*x)-...
                   ...
                   ((R*x)^2))-((b-((a*(exp((k1*(x/Tc))+(k2*(log(x/Tc)))+...
                   (k3*(1-((x/Tc)^0.5))))))/(R*x)))/x);
