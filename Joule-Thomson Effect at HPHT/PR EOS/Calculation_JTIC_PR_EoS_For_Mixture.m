function P=Calculation_JTIC_PR_EoS_For_Mixture(alpha_function,T,Tc,Pc,w,N,Yi,Kij)
...
sxV=sum(sxV(:));a_alpha_m=sxV;
    ...
    F1=@(x) (T*((R/(x-b_m))-((F_D_a_alpha_m)/((x^2)+(2*b_m*x)-(b_m^2)))))...
        +(x*((((2*a_alpha_m*(x+b_m))/(((x^2)+(2*b_m*x)-(b_m^2))^2)))-...
        ((R*T)/((x-b_m)^2))));
    
    V=fzero(F1,x0);
    
    P=((R*T)/(V-b_m))-((a_alpha_m)/((V^2)+(2*b_m*V)-b_m^2));
