function P=Calculation_JTIC_SRK_EoS_For_Mixture(alpha_function,T,Tc,Pc,w,N,Yi,Kij,Zc)
...
sxV=sum(sxV(:));a_alpha_m=sxV;
    ...
    F1=@(x) (T*((R/(x-b_m))-((F_D_a_alpha_m)/(x*(x+b_m)))))...
        +(x*((((a_alpha_m*((2*x)+b_m))/((x*(x+b_m))^2)))-...
        ((R*T)/((x-b_m)^2))));
    
    V=fzero(F1,x0);
    
    P=((R*T)/(V-b_m))-((a_alpha_m)/(V*(V+b_m)));
