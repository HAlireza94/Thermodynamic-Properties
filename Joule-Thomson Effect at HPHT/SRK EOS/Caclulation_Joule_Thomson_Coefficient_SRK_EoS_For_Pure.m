function J_T=Caclulation_Joule_Thomson_Coefficient_SRK_EoS_For_Pure...
    (alpha_function,Tc,Pc,w,T,P,A,B,C,D,E,Zc,a0)
...
R=8.3144598;
 ...
 switch lower(alpha_function)
       ...
       n2=(1/m)*(0.2469+(0.7495*w));
        ...
        F=@(x) ((R*T)/(x-b))-((a*((exp(m*(1-((T/Tc)^n2))))^2))/(x*(x+b)))-P;
     ...
     case 'trebble'
        ...
        second_order_alpha=((q1/Tc)^2)*alpha;
         ...
         F=@(x) ((R*T)/(x-b))-((a*(exp(q1*(1-Tr))))/(x*(x+b)))-P;
    ...
    second_order_alpha=((2*Tc*b1)/(T^3))+...
     ...
     F=@(x) ((R*T)/(x-b))-((a*((((b1/(T/Tc))+(b2/...
        ((T/Tc))^2)+(b3/((T/Tc))^3)))))/(x*(x+b)))-P;
        ...
        second_order_alpha=(-k*((-k*sqrt(T)-...
                         ...
                         F=@(x) ((R*T)/(x-b))-((a*((1+(k*(1-(Tr^0.5))))^2))/(x*(x+b)))-P;
     ...
     case 'jub'
         k=0.4810+(1.5963*w)-(0.2963*w^2)+(0.1223*w^3);
        alpha=((1+(k*(1-(Tr^0.5))))^2);
        first_order_alpha=-(k/T)*(sqrt(Tr))*(sqrt(alpha));
         second_order_alpha=(-k*((-k*sqrt(T)-...
                            (sqrt(Tc)*sqrt(alpha)))))/(2*Tc*(T^1.5));
...
%     Z=(P*OF)/(R*T);
    ...
    Cv=Cv_star+(T*...
     ...
     first_order_P_T=(R/(V-b))-...
         ((a*first_order_alpha)/(V*(V+b)));
    
     first_order_P_V=((-R*T)/((V-b)^2))+...
         (a*alpha*((2*V)+b))/((V^2)*((V+b)^2));
