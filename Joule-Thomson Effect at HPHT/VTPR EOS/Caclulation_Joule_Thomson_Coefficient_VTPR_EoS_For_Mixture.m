function [J_T_m, V]=Caclulation_Joule_Thomson_Coefficient_VTPR_EoS_For_Mixture...
...
second_order_alpha(i)=x4(i)+x5(i)+x6(i);
        ...
        case 'saffari'
            ...
            k1=zeros(1,N);y1=zeros(1,N);
            k2=zeros(1,N);y2=zeros(1,N);
            k3=zeros(1,N);
            alpha=zeros(1,N);second_order_alpha=zeros(1,N);
            first_order_alpha=zeros(1,N);
...
sxV=sum(sxV(:));a_alpha_m=sxV;
    ...
    F=@(x) ((R*T)/(x+c_m-b_m))-((a_alpha_m)/...
             ((x+c_m)*(x+c_m+b_m)+(b_m*(x+c_m-b_m))))-P;
...
%     Z=(P*OF)/(R*T);
     ...
     Cv_m=Cv_star_m+(T*S_D_a_alpha_m)*...
    (log((V+c_m+((1+sqrt(2))*b_m))/(V+c_m+((1-sqrt(2))*b_m))));

 
    first_order_P_T_m=(R/(V+c_m-b_m))-((F_D_a_alpha_m)/((V+c_m)*... % the problem was here.
         (V+c_m+b_m)+(b_m*(V+c_m-b_m))));
    
    first_order_P_V_m=((-R*T)/((V+c_m-b_m)^2))+...
         (2*a_alpha_m*(b_m+c_m+V))/...
         (((-b_m^2)+(2*c_m*b_m)+(2*b_m*V)+(c_m^2)+(2*c_m*V)+(V^2))^2);
