function [J_T, Speed_Sound, Cp, Cv]=Caclulation_Joule_Thomson_Coefficient_VTPR_EoS_For_Pure...
...
c=(-0.252*((R*Tc)/Pc)*((1.5448*Zc)-0.4024));
 switch lower(alpha_function)
        ...
        first_order_alpha=-(k/T)*(sqrt(Tr))*(sqrt(alpha));
         ...
         F=@(x) ((R*T)/(x+c-b))-((a*((1+(k*(1-(Tr^0.5))))^2))/...
             ((x+c)*(x+c+b)+(b*(x+c-b))))-P;
        ...
        F=@(x) ((R*T)/(x+c-b))-((a*(exp(k1*(1-(T/Tc)))))/((x+c)*(x+c+b)+(b*(x+c-b))))-P;
...
second_order_alpha=x4+x5+x6;
          ...
          F=@(x) ((R*T)/(x+c-b))-((a*(((exp(k1*(1-(T/Tc)))))*...
              ((1+(k2*(1-sqrt(Tr))^2)+(k3*(1-sqrt(Tr))^3))^2)))/((x+c)*(x+c+b)+(b*(x+c-b))))-P;
...
first_order_alpha=((-(exp(1-(n2^(log(Tr)))))*(T^(log(n2)-1))*(log(n2)))/(Tc^(log(n2))));
    ...
    (Tr^(log(n2)))*((log(n2))-1-((log(n2))*(Tr^(log(n2))))))/(T^2));
            ...
            F=@(x) ((R*T)/(x+c-b))-((a*(exp(1-(n2^(log(Tr))))))/...
             ((x+c)*(x+c+b)+(b*(x+c-b))))-P;
        ...
        case 'saffari'
            ...
            F=@(x) ((R*T)/(x+c-b))-((a*(exp((k1*Tr)+(k2*(log(Tr)))+(k3*(1-(Tr^0.5))))))/...
             ((x+c)*(x+c+b)+(b*(x+c-b))))-P;
...
%     Z=(P*OF)/(R*T);
      ...
      Cv=Cv_star+(T*a*second_order_alpha)*...
    (log((V+c+((1+sqrt(2))*b))/(V+c+((1-sqrt(2))*b))));


     first_order_P_T=(R/(V+c-b))-((a*first_order_alpha)/((V+c)*... % the problem was here.
         (V+c+b)+(b*(V+c-b))));
    
     first_order_P_V=((-R*T)/((V+c-b)^2))+...
         (2*a*alpha*(b+c+V))/...
         (((-b^2)+(2*c*b)+(2*b*V)+(c^2)+(2*c*V)+(V^2))^2);
