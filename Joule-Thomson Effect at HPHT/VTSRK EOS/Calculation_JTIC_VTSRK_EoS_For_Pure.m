function [Pr, Tr]=Calculation_JTIC_VTSRK_EoS_For_Pure(alpha_Function,p,Pc,Tc,T,w,Zc)
...
c1=(-45.7247*((1/3)-Zc));
  ...
  c=((((1/3)-Zc)*((R*Tc)/Pc))*c2);
    ...
    F=@(x) (T*((R/(x+c-b))-...
        (a*first_order_alpha/((x+c)*(x+c+b)))))...
        +(x*(((-R*T)/((x+c-b)^2))+...
        ((a*alpha*((2*c)+(2*x)+b)))/((((x+c)*(x+c+b)))^2)));
    
    V=fzero(F,x0);
    
    P=((R*T)/(V+c-b))-((a*alpha)/((V+c)*(V+c+b)));
