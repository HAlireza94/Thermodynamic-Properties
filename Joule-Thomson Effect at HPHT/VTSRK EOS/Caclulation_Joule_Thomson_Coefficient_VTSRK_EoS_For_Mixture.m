function [J_T_m, V]=Caclulation_Joule_Thomson_Coefficient_VTSRK_EoS_For_Mixture...
...
%     Z=(P*OF)/(R*T);
    ...
    first_order_P_T_m=(R/(V+c_m-b_m))-...
        (F_D_a_alpha_m/((V+c_m)*(V+c_m+b_m)));
    
    first_order_P_V_m=((-R*T)/((V+c_m-b_m)^2))+...
        ((a_alpha_m*((2*c_m)+(2*V)+b_m)))/((((V+c_m)*(V+c_m+b_m)))^2);
