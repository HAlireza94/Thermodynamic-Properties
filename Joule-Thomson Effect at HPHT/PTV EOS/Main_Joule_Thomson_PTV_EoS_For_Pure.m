...
for i=1:j
   ...
   [J_T(i,s), Speed_Sound(i,s), Cp(i,s), Cv(i,s)]=Caclulation_Joule_Thomson_Coefficient_PTV_EoS_For_Pure...
       (Tc,Pc,w,T(i),P(s),A,B,C,D,E,Zc,a0,Mw);
...
figure (1)
plot(T,jt);title('Joule_Thomson')
figure (2)
plot(T,Cp);title('Isobaric Heat Capacity (Cp)')
figure (3)
plot(T,Cv);title('Isochoric Heat Capacity (Cv)')
figure (4)
plot(T,Speed_Sound);title('Speed of Sound (v)')

% if numel(T)==300
% plot(T,jt)
% legend({'P=300 atm','P=360 atm'})
% else 
%     plot(T,jt,'+');legend({'P=300 atm','P=360 atm'})
% end
% 
% M360=jt(:,2);
% M300=jt(:,1);