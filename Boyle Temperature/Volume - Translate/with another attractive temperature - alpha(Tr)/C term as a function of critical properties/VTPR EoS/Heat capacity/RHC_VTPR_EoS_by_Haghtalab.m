...
for i=1:n
     ...
     f=@(x) ((R*T(i))/(x+c(i)-b(i)))-((a(i)*(exp(1-(n2(i)^(log(T(i)/Tc(i))))))...
         )/((x+c(i))*(x+c(i)+b(i))...
         +(b(i)*(x+c(i)-b(i)))))-P;
...
for i=1:n
    ...
    first_order_alpha(i)=((-alpha(i)*log(n2(i)))/T(i))*(1-(log(alpha(i))));
    
    second_order_alpha(i)=(log(n2(i)))*(((alpha(i)*(1-alpha(i)))/power(T(i),2))-...
    (((exp(1-((2*n2(i))^(log(Tr(i))))))*(T(i)^((log(n2(i)))-2))*...
    (log(n2(i)))*((2*exp(1))-(exp(n2(i)^(log(Tr(i)))))))/(Tc(i)^(log(n2(i))))));
