...
for i=1:n
     ...
     f=@(x) ((R*T(i))/(x-b(i)))-...
         ((a(i)*(exp(k(i)*(1-(T(i)/Tc(i))))))/((x^2)+(2*b(i)*x)-(b(i)^2)))-P;
...
J_T=zeros(n,1);Tr=zeros(n,1);
Cv=zeros(n,1);Cp=zeros(n,1);
Gamma=zeros(n,1);X=zeros(n,1);
RSS=zeros(n,1);sound_speed=zeros(n,1);
